...
### 阶段 4：推广组件工厂（优先级：🟢 低 - 后续开发中逐步迁移）

**目标**：鼓励使用 UIButtonFactory 创建按钮。

**当前状态**：低优先级，可在后续开发中逐步迁移

**收益/成本比**：较低，当前按钮功能正常，改动收益有限

**建议**：作为后续开发的一部分，当需要创建新按钮时使用 UIButtonFactory
...
- [x] COLORS 只在 ui_constants.py 中定义
- [x] 所有间距使用 SPACING 常量（主窗口已一致）
- [x] 内联样式减少 80%（主要对话框已修复）
- [ ] 推广使用 UIButtonFactory（低优先级，后续开发中逐步迁移）
- [ ] 单元测试全部通过
- [ ] UI 行为无变化
